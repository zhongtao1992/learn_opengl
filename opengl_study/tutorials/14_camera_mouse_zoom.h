...
static int tutorial14() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
			...
			// forward
			cameraPos += g_cameraFront * moveSpeed;
		}
			...
			// left
			cameraPos += glm::normalize(glm::cross(g_cameraFront, cameraUp)) * moveSpeed;
		}
		else if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
			// right
			cameraPos += glm::normalize(glm::cross(cameraUp, g_cameraFront)) * moveSpeed;
		}
		...
		// projection matrix
		glm::mat4 projection = glm::perspective(glm::radians(g_fov), g_viewWidth / g_viewHeight, 0.1f, 100.0f);
