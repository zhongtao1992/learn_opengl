...
static int tutorial38() {
	...
	program.initWithDiskFile("shader/38_normal_visualization.vert", "shader/38_normal_visualization.frag");
	GLShaderProgram normalVisualProgram;
	normalVisualProgram.initWithDiskFile("shader/38_normal_visualization_fornormal.vert", "shader/38_normal_visualization_fornormal.frag", "shader/38_normal_visualization.geom");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		normalVisualProgram.setMatrix4fv("u_projection", glm::value_ptr(projection));
		normalVisualProgram.setMatrix4fv("u_view", glm::value_ptr(view));
		// render the loaded model show normal
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(0.0f, -1.75f, 0.0f)); // translate it down so it's at the center of the scene
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));	// it's a bit too big for our scene, so scale it down
		normalVisualProgram.setMatrix4fv("u_model", glm::value_ptr(model));
		renderModel.Draw(normalVisualProgram);
