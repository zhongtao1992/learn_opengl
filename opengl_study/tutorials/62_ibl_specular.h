...
static int tutorial62() {
	...
	program.initWithDiskFile("shader/62_pbr.vert", "shader/62_pbr.frag");
	...
	GLShaderProgram prefilterShader;
	prefilterShader.initWithDiskFile("shader/60_cubemap.vert", "shader/62_prefilter.frag");
	GLShaderProgram brdfShader;
	brdfShader.initWithDiskFile("shader/62_brdf.vert", "shader/62_brdf.frag");
	...
	prefilterShader.setMatrix4fv("u_projection", captureProjection);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
	...
	for (unsigned int mip = 0; mip < maxMipLevels; ++mip){
		...
		for (unsigned int i = 0; i < 6; ++i){
			prefilterShader.setMatrix4fv("u_view", captureViews[i]);
