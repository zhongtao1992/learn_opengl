...
static int tutorial24() {
	...
	program.initWithDiskFile("shader/24_material_object.vert", "shader/24_material_object.frag");
	...
	program.setFloat("u_light.constant", 1.0f);
	program.setFloat("u_light.linear", 0.09f);
	program.setFloat("u_light.quadratic", 0.032f);
	program.setVec3("u_light.position", lightPos.x, lightPos.y, lightPos.z);
	glm::vec3 front = mCamera.getFront();
	program.setVec3("u_light.direction", lightPos.x, lightPos.y, lightPos.z - 5);
	program.setFloat("u_light.cutOff", glm::cos(glm::radians(12.5f)));
	program.setFloat("u_light.outerCutOff", glm::cos(glm::radians(17.5f)));
