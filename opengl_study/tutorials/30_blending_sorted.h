...
static int tutorial30() {
	...
	program.initWithDiskFile("shader/30_blending_sorted.vert", "shader/30_blending_sorted.frag");
	...
	ImageLoader windowImg;
	windowImg.loadImg("res/blending_transparent_window.png");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// grass
		glBindVertexArray(transparentVAO);
		glBindTexture(GL_TEXTURE_2D, windowImg.getID());
		// 
		/*for (unsigned int i = 0; i < vegetation.size(); ++i) {
			model = glm::mat4(1.0f);
		...
		// use sorted, reverse render
		for(auto ite = sorted.rbegin(); ite != sorted.rend(); ++ite){
			model = glm::mat4(1.0f);
