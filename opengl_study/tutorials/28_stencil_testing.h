...
static int tutorial28() {
	...
	program.initWithDiskFile("shader/27_depth_testing.vert", "shader/27_depth_testing.frag");
	GLShaderProgram singleColor;
	singleColor.initWithDiskFile("shader/28_shader_singlecolor.vert", "shader/28_shader_singlecolor.frag");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		singleColor.setMatrix4fv("u_projection", glm::value_ptr(projection));
		singleColor.setMatrix4fv("u_view", glm::value_ptr(view));
		...
		// draw floor normal, dot not write stencil buffer by set mask 0x00
		glStencilMask(0x00);
		// floor
		glBindVertexArray(planeVAO);
		...
		// render pass, draw object as noraml, wirte to the stencil buffer
		...
		// draw scaled object,disable stencil testing
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
		glStencilMask(0x00); // same effect with glDisable(GL_STENCIL_TEST)
