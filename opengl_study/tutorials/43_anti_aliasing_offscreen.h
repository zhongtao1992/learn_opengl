...
// anti multisample off-screen
static int tutorial43() {
	int SAMPLE_COUNT = 4;
	// config opengl
	GLFWwindow* window = nullptr;
	if (commonInitWithMultiSample(&window, SAMPLE_COUNT) < 0) {
		return -1;
	}
	...
	GLShaderProgram screenShader;
	screenShader.initWithDiskFile("shader/32_screen_shader.vert", "shader/32_screen_shader.frag");
	...
	glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, SAMPLE_COUNT, GL_RGB, g_viewWidth, g_viewHeight, GL_TRUE);
	...
	glRenderbufferStorageMultisample(GL_RENDERBUFFER, SAMPLE_COUNT, GL_DEPTH24_STENCIL8, g_viewWidth, g_viewHeight);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, g_viewWidth, g_viewHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, g_viewWidth, g_viewHeight);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, crbo);
	// check
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
		std::cout << "ERROR::FRAMEBUFFER:: FrameBuffer is not complete!" << std::endl;
	}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glBlitFramebuffer(0, 0, g_viewWidth, g_viewHeight, 0, 0, g_viewWidth, g_viewHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
