...
void main(){
    ...
    vec4 tex = texture(u_material.specular, oTexCoords);
    // reverse the sampled specular color
    //vec3 specular = u_light.specular * (spec * vec3( 1 - tex.x, 1 - tex.y, 1 - tex.z));
    vec3 specular = u_light.specular * spec * (vec3(1.0) - vec3(texture(u_material.specular, oTexCoords)));
